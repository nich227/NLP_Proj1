...
train_df = pd.DataFrame({'prem': train.prem, 'hyp': train.hyp, 'lab': train.lab})
train_tns = torch.tensor(train_df.values).float().to(device)

# Initialize TensorDataset and DataLoader
train_tns = TensorDataset(train_tns)
