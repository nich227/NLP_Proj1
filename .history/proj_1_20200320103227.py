...
x_df = pd.DataFrame({'prem': train.prem, 'hyp': train.hyp})
y_df = pd.DataFrame({'lab': train.lab})
x_train_tns = torch.tensor(x_df.values).float().to(device)
y_train_tns = torch.tensor(y_df.values).float().to(device)
